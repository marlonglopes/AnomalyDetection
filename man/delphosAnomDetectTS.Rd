delphos_format_timestamp <- function(indf, index = 1) {
...
delphos_get_gran = function(tsdf, index=1) {
  n = length(tsdf[[index]])
...
delphos_detect_anoms <- function(data, k = 0.49, alpha = 0.05, num_obs_per_period = NULL,
  ...
  if(posix_timestamp){
    data_decomp <- delphos_format_timestamp(data_decomp)
  }
...
delphos_anomalyDetectionTs <- function(x, max_anoms = 0.10, direction = 'pos',
  ...
  if(gran == "sec"){
    x <- delphos_format_timestamp(aggregate(x[2], format(x[1], "%Y-%m-%d %H:%M:00"), eval(parse(text="sum"))))
    gran = "min"
  }
  ...
  for(i in 1:length(all_data)) {
    ...
    s_h_esd_timestamps <- delphos_detect_anoms(all_data[[i]], k=max_anoms, alpha=alpha, num_obs_per_period=period, use_decomp=TRUE, use_esd=FALSE,
